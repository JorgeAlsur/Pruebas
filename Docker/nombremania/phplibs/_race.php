...
// +----------------------------------------------------------------------+
// | race.php  - RACE encode and decode strings.                          |
// +----------------------------------------------------------------------+
// | Authors: Allan Hansen  <All@nHansen.dk>                         |
// |          Arjan Wekking <a.wekking@synantics.nl>                      |
// +----------------------------------------------------------------------+
//
// $Id: race.php,v 1.2 2003/07/23 13:23:33 ah Exp $
...
class Net_RACE extends PEAR {
    ...
    var $raceUMap = array(
        ...
        0x00B5 => array("\x00\x00\x03\xBC", 'Case map'), // µ: bq--ac2q -- ALLANH  (\x00\x00\x00\xB5) ?
        ...
        0x00DF => array("\x00\x00\x00\xDF", 'Case map'), // ß: bq--adpq -- ALLANH              // 0x00DF => array("\x00\x00\x00\x73\x00\x00\x00\x73", '73'), 
    ...
    */
    ...
    { 
        return eregi("^[-a-z0-9]+$", $inString);       // ALLANH
    }
...
////////////////////////////////////////////////////////////////////////////////
/// Shortcut functions by Allan Hansen /////////////////////////////////////////
////////////////////////////////////////////////////////////////////////////////
...
* RACE encode string.
*
* @param    string  Domain  String to RACE encode.
*/

function RaceEncode($Domain)
{ 
    if (!$Domain)
        return;
    ...
    $str    = array_shift($domain);
  
    // domain only contains plain characters
    if (eregi("^[-a-z0-9]+$", $str)) 
        return $Domain;
    
    // race convert
    if (!eregi("^[-a-z0-9¡¢£¤¥¦§¨©ª«¬®¯°±²³´¶·¸¹º»¼½¾¿ÀÁÂÃÄÅÆÇÈÉÊËÌÍÎÏÐÑÒÓÔÕÖ×ØÙÚÛÜÝÞßàáâãäåæçèéêëìíîïðñòóôõö÷øùúûüýþÿ]+$", $str))
        die("Internal Error: RaceEncode() Illegal character in domain name.");
    
    $RACE = new Net_RACE("ISO-8859-1");
    $RACE->doRace($str);

    if($RACE->raceConverted) { 
        array_unshift($domain, $RACE->raceResult);
        return implode(".", $domain);
    } else
        return "RACE error: ".$RACE->raceError;  
}
...
* RACE decode string.
*
* @param    string  Domain  String to RACE decode.
*/

function RaceDecode($Domain)
{ 
    if (!$Domain)
        return;
 
    // domain is not race encoded...
    if (!eregi("^bq--", $Domain)) 
        return $Domain;    
