...
function is_email($string)
{
    ...
    $string = trim($string);
  	...
  	$ret = preg_match('/^[A-Z0-9._%-]+@[A-Z0-9][A-Z0-9.-]{0,61}[A-Z0-9]\.[A-Z]{2,6}$/i', $string);
	return $ret;
}
...
{
    $ret = _is_valid($string, $min_length, $max_length, "[a-zA-Z[:space:]ÀÁÂÃÄÅÆÇÈÉÊËÌÍÎÏĞÑÒÓÔÕÖØÙÚÛÜİŞßàáâãäåæçèéêëìíîïğñòóôõöøùúûüış`´']+");
...
list($h,$m)=explode(':',$s);
$ret=true;
if ($h<0 or $h>23) $ret=false;
if ($m<0 or $m>59) $ret=false;
