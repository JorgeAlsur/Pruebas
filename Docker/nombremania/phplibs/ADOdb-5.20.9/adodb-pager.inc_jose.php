...
class ADODB_Pager {
	...
	function RenderControles($rows=10)
	{
		...
		{
			$rs=&$this->db->CachePageExecute($this->cache,$this->sql,$rows,$this->curr_page);
			if($debug)echo "cache<br/>";
		}
		else
		{
			$rs=$this->db->PageExecute($this->sql,$rows,$this->curr_page);
			if($debug)echo $this->sql." nocache<br/>";
			//print_r($rs);exit;
		}
		$ADODB_COUNTRECS=$savec;
		...
		/*echo $this->sql." $rows ".$this->curr_page;
		...
		if(!$rs->EOF && (!$rs->AtFirstPage() || !$rs->AtLastPage()))$header=$this->RenderNav2(true);
		else $header="&nbsp;<!-- no controles -->";
		return $header;
	}
	...
	function set_params($ar)
	{
		...
		foreach($ar as $k=>$v)
		{
			...
			// (10-09-2009)
			//
			// Para que no aparezcan varios parametros adodb_next_page en la URL
			// no se incluye el parametro en el array.
			if($k!='adodb_next_page')
			{
				$x[]=$k."=".$v;
			}
		}
		...
		// (10-09-2009)
		//
		// Anadida una comprobacion de si se han recibido parametros
		// para evitar un error.
		if(isset($x)) $this->param_vars=implode('&', $x);
		return;
	}
}
