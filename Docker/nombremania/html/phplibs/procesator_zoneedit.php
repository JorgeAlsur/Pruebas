...
function a_registrados($id_cola,$id_solicitud,$dominio)
{
	...
	else
	{
		...
		{
			//para renovaciones se da de alta la operacion pero las no zonas ni dominios se actualizan los datos en origen
			$updcola=$conn->execute("update cola set estado=100 where id=$id_cola");
			if($updcola)
			{
				echo "nada en operaciones, todo ok";
			}		 
		}
	}
	if($debug)mail("jose@alsur.es","debug",$debug_string,"Content-Type: text/html");
...
function procesa_expired()
{
	...
	$sql="select * from cola where estado = 77;";
	$rs=$conn->execute($sql);
	...
	while(!$rs->EOF)
	{
		...
		//mail('jose@alsur.es','Cola Expired System Debug',"$dominio -> $result");
		...
		if(substr($result,0,3)=='210')
		{
			...
			$conn->execute($sql);
			$ok=1;
			mail('sistemas@alsur.es','Registro expired',"El dominio $dominio, que estaba a punto de expirar, ha sido registrado con éxito.");
		}
		...
		if($tmp_rs->_numOfRows<1)
		{
			...
			$conn->execute($sql);
			mail('sistemas@alsur.es','Registro expired',"El dominio $dominio debe haber sido renovado o reregistrado, ya que no se ha conseguido registrarlo en el plazo de tiempo especificado.");
		}
		//if($ok)procesa_cola();
		$rs->movenext();
	}
