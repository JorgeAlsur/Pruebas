...
//include_once($_SERVER['DOCUMENT_ROOT']."/gisgestion/conf.php");
...
class Template
{
	...
	var $filename="";
	//var $cache_dir="/home/webs/golfinspain.com/html/php/cachegis";
	var $cache = false;
	var $cache_dir = false;
    ...
    function parse($target, $block = "", $append = false) {
        ...
        if(isset($this->blocks["/\{$block}/"])) {
            ...
            switch($this->unknowns) {
				...
				//echo "Default: $target<br/> ".$this->vars["/\{$target}/"]."<br/>";
                break;
            }
        } else {
    ...
    function load_file_OLD($filename) {
        ...
        if($this->DEBUG) { print("<li><b>_load_file</b></li>\n".var_crudas(func_get_args())); }
				if(file_exists($_SERVER['DOCUMENT_ROOT']."/".$filename) ){
					 $auxdir=		$_SERVER['DOCUMENT_ROOT']."/";
					 $fh = fopen($_SERVER['DOCUMENT_ROOT']."/".$filename, "r");
				   $file_content = fread($fh, filesize($_SERVER['DOCUMENT_ROOT']."/".$filename));
					 fclose($fh);
			} 
...
//		ob_implicit_flush(1);
    ...
    function extract_blocks($name, $block) {
		...
		if(!isset($this->blocks["/\{$current_block}/"]))$this->blocks["/\{$current_block}/"]='';
        if(list(, $block) = @each($blocks)) {
            ...
            while(list(, $block) = @each($blocks)) {
                ...
                switch($regs[1]) {
                    ...
                    case "END":
                    if(($level-1)<0)$level--;
					else $current_block = $block_names[--$level];
					if(!isset($this->blocks["/\{$current_block}/"]))$this->blocks["/\{$current_block}/"]='';
                    $this->blocks["/\{$current_block}/"] .= $regs[3];
                    break;
...
function get_block($bloque)
{
	...
	function cache_template($target)
	{
		...
		{
			mail("jose@alsur.es","Debug System cache_template","No existe el directorio ".$this->cache_dir."/".$this->cache_id.".cache","From: Debug System <jose@alsur.es>\nContent-Type: text/html");
			return false;
		}
		//else die("error en el directorio o al grabar los caches en ".$this->cache_dir."/".$this->cache_id.".cache");
	}
