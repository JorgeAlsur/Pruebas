...
require_once("osrsh/XML_Codec.php");
...
class openSRS_base extends PEAR
{
	...
	var $OPENSRS_TLDS_REGEX = '(\.ca|\.(bc|ab|sk|mb|on|qc|nb|ns|pe|nf|nt|nv|yk)\.ca|\.com|\.net|\.org|\.co\.uk|\.org\.uk|\.tv|\.vc|\.cc|\.info|\.biz|\.name|\.es|\.us)';
...
#
	...
	function init_socket() {
		...
		$connection_protocol = '';
        if ('SSL' == $this->crypt_type) {
        	/* ssl:// requires PHP v4.3.0+ compiled with --with-openssl  */
        	$connection_protocol = 'ssl://';
        }
		$this->_socket = fsockopen($connection_protocol.$this->REMOTE_HOST, $this->REMOTE_PORT,
...
# END of new error stuff
	...
	function lookup_domain_race($lookupData)
	{
		$domain = strtolower($lookupData['attributes']['domain']);
		if ($domain=='') {
			$data = array(
				'is_success'	=> false,
				'response_code'	=> 490,
				'response_text'	=> "Sintaxis invalida, debe poner al menos un dominio."
			);
			return $data;
	    	}
		$syntaxError = $this->check_domain_syntax($domain);
...
# END of new error stuff
			...
			$data = array(
				'is_success'	=> false,
				'response_code'	=> $code,
				'response_text'	=> "Sintaxis invalida para dominio $domain: $syntaxError."
			);
			return $data;
	    	}
		...
		foreach($domains as $local_domain) {
			$lookupData['attributes']['domain'] = $local_domain;
			# send request to server
			//horaciod chequeo de dns previo al lookup
/*			if ( !stristr($local_domain , "tv") and !stristr($local_domain , "cc") and !$anula_checkdnsrr ){
#and  checkdnsrr("$local_domain","ANY")
				$data['is_success']	= 1;
				$data['response_code']	= 200;
				$data['response_text']	= "Domain Taken - dns $local_domain" ;
				$data['attributes']['status'] = "taken";
//	 			echo "<br>tomado $local_domain";
						}else {
 */
 		$this->send_data( $lookupData );
		$answer = $this->read_data( );
//		print_r($answer);
			...
			if ($local_domain==$domain) {
				...
				$data['attributes']['upg_to_subdomain'] = $answer['attributes']['upg_to_subdomain'];
				$data['attributes']['reason'] = $answer['attributes']['reason'];
				if (isset($answer['attributes']['price_status'] ) and $answer['attributes']['price_status']=="AUCTION" )	{ 
				//agrega los precios para los TV
				$data['attributes']['status'] = "subasta";
				$data["attributes"]['price_status']=$answer['attributes']['price_status'];
				$data["attributes"]['price']=$answer['attributes']['price'];
				}
			}
		}
//}
...
#
#	validate_nm()
#		Check data for validity de nombremania
#

function validate_nm($registrando,$validar_tecnico=false)
{
	$error_msg="";
		# Country codes are needed for checking ... more reliable than /^[A-Z]{2}$/
		include 'osrsh/country_codes.php';
		$missing_fields = $problem_fields = array();
...
if ($validar_tecnico) {
   $contact_types = array (
		...
		# check the required fields
$data=array();
  while (list($k,$v) = each($contact_types)){
include_once("String_Validation.inc.php");
			foreach ($required_contact_fields as $field=>$required_field) {

				$data[$k.'_'.$field] = trim($registrando["contactos"][$k][$field]);

				if ($data[$k.'_'.$field] == '') {
				
					$missing_fields[] = $v.' '.$required_field;
				}
				if (!is_clean_text($data[$k.'_'.$field]) && $field!='phone' && $field!='email'){
				//echo " $field entra=".$data[$k.'_'.$field];exit;
					$missing_fields[] = $v.' '.$required_field;

				}
			}

			$data[$k.'_country'] = strtolower($registrando['contactos'][$k]['country']);

			if (strtolower($data[$k.'_country']=='us') || strtolower($data[$k.'_country']=='ca')) {
				if ($data[$k.'_postal_code']=='') {
					$missing_fields[] = $v.' Codigo postal ';
				}
    
				if ($data[$type.'_state']=='') {
					$missing_fields[] = $v.' Ciudad o provincia';
				}
			}

			if (!isset($COUNTRY_CODES[$data[$k.'_country']])) {
				$problem_fields[$contact_type.' Country'] = $data[$k.'_country'];
			}

			if (!$this->check_email_syntax($data[$k.'_email'])) {
				$problem_fields[$contact_type.' Email'] = $data[$k.'_email'];
			}
			
    $otros_problemas=array();
         if (!valida_telefono($data[$k."_phone"])){
           	$otros_problemas[]= "El formato correcto para los tel&eacute;fonos es +codigo_pais.numero_de_telefono (ej:+34.952586623)";;
			}
			if ($data[$k."_fax"]<>"" and !valida_telefono($data[$k."_fax"])){
			     	$otros_problemas[]= "El formato correcto para el fax es +codigo_pais.numero_de_telefono (ej:+34.952586623)";;
			}
		}

#   solamente caracteres alfanumericos
//lo cambio a la validación original.
  	foreach ($data as $field=>$value) {
			if ($value=='') {
				continue;		# skip blanks
			}
			...
			if ($field=='first_name' || $field=='last_name' || $field=='org_name' || $field=='city' || $field=='state') {
				if (!preg_match('/[a-zA-Z]/', $value)) {
					$error_msg .= "El campo: <b>$field</b> debe contener al menos un caracter alfab&eacute;tico.<br>\n";
		...
		foreach ($missing_fields as $field) {
			$error_msg .= "El campo <b>$field</b> es requerido o contiene caracteres no aceptados.<br>\n";
		}

 		# print error if $problem_fields
		
		/*print_r($missing_fields);
		print_r($problem_fields);
		print_r($otros_problemas);
		exit;*/
		...
		if (count(array_keys($problem_fields))) {
			foreach ($problem_fields as $field=>$problem) {
				...
				# would have been caught above.
				if ($problem != '') {
					$error_msg .= "El campo <b>$field</b> contiene caracteres
                 inválidos o bien no posee una sintaxis correcta:
                 <i>$problem</i><br>\n ";
				}
			}
		}
         foreach($otros_problemas as $prob){
         $error_msg.="$prob";
         }
		...
		if ($error_msg<>"")
		{
			return ( array('error_msg' => $error_msg,'is_success' =>0) );
		} else {
...
function valida_telefono($tele)
{
	return preg_match('/^\+\d{1,3}\.\d{1,12}(x\d{1,4})?$/',$tele);
}
