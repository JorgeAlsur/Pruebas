...
class openSRS_base extends PEAR
{
	...
	var $OPENSRS_TLDS_REGEX = '(\.ca|\.(bc|ab|sk|mb|on|qc|nb|ns|pe|nf|nt|nv|yk)\.ca|\.com|\.net|\.org|\.co\.uk|\.org\.uk|\.tv|\.vc|\.cc|\.info|\.biz|\.name|\.eu|\.es|\.us)';
...
include_once("String_Validation.inc.php");
			foreach ($required_contact_fields as $field=>$required_field) {
				...
				if (!is_clean_text_strict($data[$k.'_'.$field])){
					$missing_fields[] = $v.' '.$required_field;
...
#   solamente caracteres alfanumericos
  	foreach ($data as $field=>$value) {
			...
			if ($field=='owner_first_name' || $field=='last_name' || $field=='org_name' || $field=='city' || $field=='state') {
				if (!preg_match('/[[:alnum:]]/i', $value)) {
		...
		if (count(array_keys($problem_fields))) {
			foreach ($problem_fields as $field=>$problem) {
				...
				if ($problem != '') {
                 ...
                 inv&aacute;lidos o bien no posee una sintaxis correcta:
                 <i>$problem</i><br>\n ";
				}
			}
		}
