...
$gSQLBlockRows=40; // max no of rows per table block
...
GLOBAL $gSQLMaxRows,$gSQLBlockRows;
    ...
    while (!$rs->EOF) {
        ...
        for ($i=0; $i < $ncols; $i++) {
			...
			case 'N':
				...
				break;
			default:
       if (eregi("^<a",trim($rs->fields[$i]))){
                                $s.="<td>".trim($rs->fields[$i])."&nbsp;</TD>";
                                }
                                else{
  ...
  if ($docnt) $s.="<tr><td colspan=$ncols align=right>".$rows." Registros</td></tr>";
       	print $s."</TABLE>\n\n";
	return $rows;
 }
