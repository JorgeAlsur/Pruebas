<?php
/*
V1.65 28 Dec 2001 (c) 2000, 2001 John Lim (jlim@natsoft.com.my). All rights reserved.
...
/*	$ADODB_SESSION_DRIVER='mysql';
	...
	$ADODB_SESSION_DRIVER='mysql';
*/
	//$ADODB_SESSION_CONNECT=":/var/lib/mysql/mysql.sock";
	$ADODB_SESSION_CONNECT="localhost";
	$ADODB_SESSION_USER ='nombremania';
	$ADODB_SESSION_PWD ='MySQL_Nombremania';
	$ADODB_SESSION_DB ='nombremania';
	$ADODB_SESSION_TBL = 'sessions';
...
{
	global $ADODB_SESS_INSERT,$ADODB_SESS_CONN, $ADODB_SESS_LIFE, $ADODB_SESSION_TBL,$ADODB_SESSION_DRIVER;
	global $__host,$__usuario,$__clave,$__bddatos;

	$expiry = time() + $ADODB_SESS_LIFE;
	
	$val = rawurlencode($val);
	$qry = "UPDATE $ADODB_SESSION_TBL SET expiry=$expiry,data='$val' WHERE sesskey='$key'";
	if(!is_object($ADODB_SESS_CONN))
	{
		if(empty($ADODB_SESSION_CONNECT))
		{
			$ADODB_SESSION_DRIVER='mysql';
			//$ADODB_SESSION_CONNECT=':/var/lib/mysql/mysql.sock';
			$ADODB_SESSION_CONNECT='localhost';
