...
{
	$error=urlencode("Usuario Inv&aacute;lido, su registro no ha sido dado de alta a&uacute;n o ha vencido su login, reintente. <a href=login.php>LOGIN</a>");
...
//    $usuario = base64_decode($_COOKIE["usuario"]);
	$id_operacion=$_COOKIE['id_nombremania'];
	//$conn->debug=true;
	$rs1=$conn->execute("select * from clientes where id=$id_cliente");
	$id_cliente=$rs1->fields['id'];
	$email=$rs1->fields['email'];
	$nombre=$rs1->fields['nombre'];
	$usuario=$rs1->fields['usuario'];
	$rs=$conn->execute("select * from solicitados where id=$id_operacion");
	$dominios=$rs->fields['domain'];
	...
	$saldo=$rs2->fields('saldo');

	$clavepasada=base64_encode("$id_operacion:nombremania:$dominios:$usuario");

	$donde="http://$SERVER_NAME/clientes/pago/realizarpago.php?codigo=$clavepasada";
//$donde="http://192.168.2.99/clientes/realizarpago.php?codigo=$clavepasada";          //solo pruebas
//print "<a href=\"$donde\">Link desde el mail</a>";
	include "enviar_email.php";
	$datos["nombre"]=$nombre;
	$datos["email_cliente"]=$email;
	$datos["saldo"]=$saldo;
	$datos["link"]=$donde;
	...
	$ruta=$DOCUMENT_ROOT;
	envioemail($ruta."/procesos/mails_a_enviar/mail_link_pago.txt",$datos);
}
?>
  ...
  <tr> 
	...
	<!-- #BeginEditable "contenido" --> 
      ...
      <table width="710" border="0" cellspacing="0" cellpadding="0">
        <tr> 
          <td width="180" valign="top"><br><!-- #BeginLibraryItem "/Library/banner180_ 2x.lbi" --><!--banner NMcajadoble de phpads -->
<iframe src='http://linux.alsur.es/phpads/adframe.php?what=zone:5&target=new&refresh=25' framespacing='0' frameborder='no' scrolling='no' width='180' height='150'><script language='JavaScript' src='http://linux.alsur.es/phpads/adjs.php?what=zone:5&target=new&withText=0'></script><noscript><a href='http://linux.alsur.es/phpads/adclick.php' target='new'><img src='http://linux.alsur.es/phpads/adview.php?what=zone:5&target=new&withText=0' border='0'></a></noscript></iframe><!-- #EndLibraryItem --></td>
          ...
          <td width="520" valign="top">
            <table border=0 width="520" align="left" cellpadding="3" class="body_10">
              <tr> 
                <td colspan="3" height="20" class="titu_pags"><font size=+1><b><font size=+1><b><font face="Verdana, Arial, Helvetica, sans-serif" size="2"><span class="titu_pags_previo">CLIENTE 
                  REGISTRADO</span></font></b></font><span class="titu_pags_previo"><font face="Verdana, Arial, Helvetica, sans-serif" size="2"></font></span><font face="Verdana, Arial, Helvetica, sans-serif" size="2"><br>
                  SOLICITUD DE PAGO</font></b></font></td>
              </tr>
              <tr> 
                <td colspan="3">
                  <p>Ha solicitado se le debite en su cuenta los conceptos que 
                    a continuaci&oacute;n se detallan.<br>
                    Hemos enviado un email a la direcci&oacute;n registrada en 
                    su cuenta que le permitir&aacute; confirmar mediante un enlace 
                    la aceptaci&oacute;n de este pago por un periodo de siete 
                    d&iacute;as.</p>
                  <p><b>RECUERDE!<br>
                    El/los dominios solicitados NO quedar&aacute;n registrados 
                    hasta que confirme este &uacute;ltimo paso.</b></p>
                  </td>
              </tr>
              <tr> 
                <td colspan="3"> 
              ...
              <tr> 
                <td colspan="3"> 
                  <table width="100%" border="0" cellspacing="2" cellpadding="2">
                    <tr> 
                      <td width="32%" bgcolor="#FFCC00" class="td_form_titulo"> 
                        <div align="right"><font face="Verdana, Arial, Helvetica, sans-serif" size="1">Cliente:</font></div>
                      </td>
                      <td width="68%" bgcolor="#FFFFFF" class="td_form_valor"><b> 
                        <?=$nombre?>
                        <? echo "($email)"; ?>
                        </b></td>
                    </tr>
                    <tr> 
                      <td width="32%" bgcolor="#FFCC00" class="td_form_titulo"> 
                        <div align="right"><font face="Verdana, Arial, Helvetica, sans-serif" size="1">Dominios:</font></div>
                      </td>
                      <td width="68%" bgcolor="#FFFFFF" class="td_form_valor"> 
                        <b> 
                        <!--recortar longitud -->
                        <?=$rs->fields["domain"]?>
                        </b><b><font face="Verdana, Arial, Helvetica, sans-serif" size="1"></font></b></td>
                    </tr>
                    <tr> 
                      <td width="32%" bgcolor="#FFCC00" class="td_form_titulo"> 
                        <div align="right"><font face="Verdana, Arial, Helvetica, sans-serif" size="1">Importe:</font></div>
                      </td>
                      <td width="68%" bgcolor="#FFFFFF" class="td_form_valor"><b> 
                        <?=$importe ?>
                        </b></td>
              ...
              <tr> 
                <td colspan="3"><font face="Verdana, Arial, Helvetica, sans-serif" size="2"><br>
                  <span class="titu_secciones_com">Gracias por usar NombreMania.com</span></font></td>
              </tr>
            </table>
          </td>
        </tr>
      ...
      <p><font face="Verdana, Arial, Helvetica, sans-serif" size="2"> 
        <!--En caso de error: lo enviamos a error.php:
Lo sentimos, usuario o password no reconocidos. Intentelo de nuevo.-->
